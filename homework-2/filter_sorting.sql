...
SELECT AVG(shipped_date - order_date) AS delivery_time FROM orders WHERE ship_country IN ('Germany')
...
SELECT MIN(unit_price) AS min_price, MAX(unit_price) AS max_price FROM products WHERE discontinued = 0
...
SELECT MIN(unit_price) AS min_price, MAX(unit_price) AS max_price FROM products WHERE discontinued = 0 AND units_in_stock >= 20
